...
abstract class NetworkThread implements Runnable {
    ...
    private ArrayList encode(String string, int retainCount) {
        ...
        CoderResult result;
        for(result = this.encoder.encode(input, firstMessage.buffer, true); result.isOverflow(); result = this.encoder.encode(input, message.buffer, true)) {
        ...
        for(result = this.encoder.flush(message.buffer); result.isOverflow(); result = this.encoder.flush(message.buffer)) {
            length += message.buffer.position();
            message.buffer.flip();
            messages.add(message = this.createMessage());
            message.retainCount = retainCount;
        ...
        while(true) {
            ...
            do {
                while(!this.messagesToWrite.isEmpty()) {
                    message = (PendingMessage)this.messagesToWrite.remove(0);
