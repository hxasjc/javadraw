...
public class TextWrappingLayout {
    ...
    private boolean doLayoutOnArc() {
        ...
        if (text.length() == 0) {
            ...
            for(run = this.start; run != null; run = run.getNextRun()) {
    ...
    private boolean doLayoutNoShape() {
        ...
        if (text.length() == 0) {
            ...
            for(layout = null; run != null; spacing = (double)((layout.getAscent() + layout.getDescent()) / 2.0F + layout.getLeading())) {
                ...
                AffineTransform transform = AffineTransform.getTranslateInstance(-layout.getBounds().getMinX() - (1.0 + this.alignH) * layout.getBounds().getWidth() / 2.0, top + (double)layout.getAscent());
                ...
                while(offset < endOffset) {
                    ...
                    if (offset != end) {
                        ...
                        transform.translate((double)runLayout.getAdvance(), 0.0);
                        offset = end;
                    }
            ...
            AffineTransform transform = AffineTransform.getTranslateInstance(0.0, -(1.0 + this.alignV) * top / 2.0);
            run = this.start;

            for(offset = 0; run != null; ++offset) {
                ...
                if (this.alignV != -1.0 && this.isFullLayout(map)) {
                    ...
                    if (bestAlign > align) {
                        ...
                        while(improved) {
                            ...
                            for(double jump = maxY - minY; !improved && jump > 2.0; jump /= 2.0) {
                                for(double y = minY + jump / 2.0; !improved && y < maxY; y += jump) {
    ...
    private ArrayList getLine(Rectangle2D line) {
        ...
        for(double jump = line.getWidth(); jump > minWidth; jump /= 2.0) {
            for(double x = jump / 2.0; x <= line.getWidth(); x += jump) {
    ...
    private Rectangle2D placeWord(double width, double height, double align) {
        ...
        while(improved) {
            ...
            for(double jump = maxY - minY; !improved && jump > 0.5; jump /= 2.0) {
                for(double y = minY + jump / 2.0; !improved && y < maxY; y += jump) {
    ...
    private void writeLayout(HashMap map, AffineTransform transform) {
        for(TextRun run = this.start; run != null; run = run.getNextRun()) {
            ...
            if (map != null && (path = (GeneralPath)map.get(run)) != null) {
                if (transform != null) {
                    path = transform.createTransformedShape((Shape)path);
                }
            } else {
                path = new GeneralPath();
            }

            run.setShape((Shape)path);
        }
        ...
        public void doLayout(int offset, double height, double top) {
            ...
            if (limit == 0) {
                ...
                while((run = run.getNextRun()) != null) {
                    int lenx = run.getText().length();
                    if (lenx > 0) {
                        string.addAttribute(TextAttribute.FONT, run.getFont(), pos, pos + lenx);
                        pos += lenx;
                    }
                }
                ...
                for(int i = 0; i < rects.size() && this.status == 0; ++i) {
                    ...
                    if (pos < limit && !((double)layoutWidth < 0.2 * height)) {
                        ...
                        if (layout != null && pos != measurer.getPosition()) {
                            ...
                            double padding;
                            if (rect.getWidth() - width < 2.0 * TextWrappingLayout.this.endPadding) {
                                padding = (rect.getWidth() - width) / 2.0;
                            } else {
                            ...
                            if (this.totalOffset > endOfRun) {
                                ...
                                while(measurer.getPosition() < this.totalOffset) {
                                    ...
                                    path = new GeneralPath();
                                    this.shapes.add(path);
                                    transform.translate((double)layout.getAdvance(), 0.0);
                                    if (len > 0) {
